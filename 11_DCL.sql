...
-- [작성법]
-- CREATE USER 사용자명 IDENTFIED BY 비밀번호;
CREATE USER jmj_sample IDENTIFIED BY 1234;

-- 2.새 연결(접속) 추가
-- ORA-01045: 사용자 JMJ_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한

-- 3. 접속 권한 부여 (sys -> 사용자 계정)
-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권하 ... TO 사용자명;
GRANT CREATE SESSION TO jmj_sample;
...
GRANT CREATE TABLE TO jmj_sample;

ALTER USER jmj_sample DEFAULT 
...
--> 해당 계정은 지정된 권한을 이용해서 특정 열할을 갖게 됨
...
GRANT CONNECT, RESOURCE TO jmj_sample;
...
REVOKE SELECT ON EMPLOYEE FROM jmj_sample;

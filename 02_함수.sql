...
-- 단일 행 함수 : N개의 값을 읽어서 연산후 N개의 결과를 반환
...
-- 지정한 위치부터 지정한 순번째로 검색되는 문자의 위치르 반환
...
-- EMPLOYEE 테이브렝서 사원명, 이메일, 이메일 중 '@' 위치 조회
...
SELECT ROUND(123.456, 1) FROM DUAL; -- 소수점 첫번째 자리까지 표기
...
SELECT SYSDATE FROM DUAL; -- 2025-03-10 12:11:12.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:11:12.584 +0900
...
-- ADD_MONTHS(날짜, 숫자) : 날짜에 순자만큼의 개월수 더함
...
-- 문자열로 변환
-- TO_CHAR(날짜, [포맷]) : 날짜형 데이터를 문제형 데이터로 변경
-- TO_CHAR(숫자, [포맷]) : 숫자형 데이터를 문제형 데이터로 변경
...
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 횽식인지 인식 시킨
...
-- 사원이름, 생년월일(1965년 10월 8일)

SELECT * FROM EMPLOYEE
-- EMP_NO(주민번호) : 621231-1985634

-- 1) 주민번호에서 - 앞글자까지 추출
SELECT EMP_NAME, SUBSTR(EMP_NO, '-') -1) AS 생년월일
FROM EMPLOYEE;

-- 추출한 생년월일을 TO_DATE 타입으로 변경 -> R패턴을 이용하여 1900년도로 변경
SELECT EMP_NAME, 
TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO,'-')-1), 'RRMMDD') AS 생년월일
FROM EMPLOYEE;

-- 3) TO_CHAR를 이요해서 문자열로 반환 -> 1962년 12월 31일
SELECT EMP_NAME, 
...
-- MEPLOYEE 테이블에서 보너스를 받으면 'O', 안받으면 'X' 조회
...
-- DECODE(계산식 | 컬럼명, 조건값1, 선택값1, 조건가2, 선택값2, ... , 아무것도 일치하지 않았을 때)
...
-- AVG(숫자가 기록된 컬럼명) : 평균
-- 전 직원 급여 평균
SELECT AVG(SALARY) FROM EMPLOYEE; -- 3047662.6086956...
...
-- 급여 최대값, 가장 최근 입사일, 알파벳 순서가 가장 빠른 이메일
...
-- COUNT() : 행 개수 반환
-- COUNT(컬럼명) : 행 개수 반환(NULL 제외)
-- COUNT(*) : 행 개수 반환(NULL 포함)
-- COUNT([DISTINCT] 컬럼명) : 중복 제거 후 행 개수 반환

...
--> VIEW를 이용해서 DML(INSERT/UPDATE/DELETE) 막기 위해서 사용
 ...
 *   [작성법]
  ...
  [MAXVALUE 숫자 | NOMAXVALUE] -- 발생시킬 최대값 지정, 생략하면 기본값은 10^27 - 1 (즉, 매우 큰 값)입니다 / NOMAXVALUE를 사용하면 최대값 제한이 없음을 의미 / 기본값은 NOMAXVALUE
  [MINVALUE 숫자 | NOMINVALUE] -- 최소값 지정 , 기본값은 -10^26 /  NOMINVALUE를 사용하면 최소값 제한이 없음을 의미 / 기본값은 NOMINVALUE
...
--> CURRVAL는 가장 최근 호출된 NEXTVAL의 값을 반환함
...
INSERT INTO TB_TEST
VALUES (SEQ_TEST_NO.NEXTVAL, '유리'); -- 130

INSERT INTO TB_TEST
VALUES (SEQ_TEST_NO.NEXTVAL, '철수'); -- 135
...
-- 인덱스가 ROWID 저장함
-- 인덱스는 컬럼의 값과 해당행의 ROWID를 매핑해서 저장
...
SELECT COUNT(*) FROM TB_IDX_TEST; -- 100만개 행 삽입 확인

-- 인덱스를 사용해서 검색하는 방법
--> WHERE 절에 INDEX가 적용된 컬럼을 언급

 ...
 * 
 * - IN / NOT IN : 여러개의 결과값 중에서 한개라도 일치하는 값이 있다면 / 없다면
 * 
...
SELECT EMP_ID, EMP_NAME, DEPT_TITE, JOB_NAME
FROM EMPLOYEE
...
-- 1) 직급이 대리인 직원들의 사번, 이름, 직급명, 급여 조회 (메인 쿼리)
...
-- 사원의 이름, 직원코드, 부서코드, 입사일 조회

-- 1) 퇴사한 여직원 조회하기
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE ENT_YN = 'Y'
AND SUBSTR(EMP_NO, 8, 1); -- D8 J6 (이태림)

-- 2) 퇴사한 여직우너과 같은 부서, 같은 직급 조회

-- 방법1) 단일행 단일열 서브쿼리 2개 사용
SELECT DEPT_CODE
FROM EMPLOYEE
WHERE ENT_YN = 'Y'
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
															FROM EMPLOYEE
															WHERE EXCEPT(YEAR FROM HIRE_DATE) = 2000);

-- 3. 77년생 여자 사원과 동일한 부서이면서
-- 동일한 사수를 가지고 있는 사원 조회
-- 사번, 이름, 부서코드, 사수, 주민번호, 입사일
...
-- 5. 상[호 연]관 서브쿼리
-- 메인쿼리가 사용하는 테이블값을 서브쿼리가 이용해서 결과를 만듦
-- 메인쿼리의 테이블값이 바뀌면 서브쿼리의 결과값도 바뀜

-- 먼저 메인쿼리 한 행을 조회하고
-- 해당 행이 서브쿼리의 조건을 충족하는지 확인하여 SELET 진행
...
-- 실제로 직원 테이브르이 EMP_ID와 일치하는지 확인

-- 메인쿼리(직원의 사번, 이름, 부서명, 사수번호 조회)
...
-- 스칼라 : 단일값(SELECT 절에 작성되는 단일행 단일열 서브쿼리)

-- 모든 직원의 이름, 직금, 급여, 전체 사원 중 가장 높은 급여와의 차를 조회
...
--> SELECT 문의 해석 순서 때문에
-- 급여 상위 5명 X / 조회 순서 상위 5명 O

-- 1) 이름, 급여를 급여 내림차순으로 조회한 결과를 인라인뷰 사용
--> FROM 절에 작성되기 때문에 해석 순위 1위

...
-- *** DEPT_COPY 의 DEPT_TILTE 컬럼에 NOT NULL 제약조건 추가 / 삭제 ***
ALTER TABLE DEPT_COPY 
ADD CONSTRAINT DEPT_COPY_TITLE_NN NOT NULL (DEPT_TITLE);
-- ORA-00904: : 부적합한 식별자
--> NOT NULL 제약조건은 새로운 조건 추가가 아닌
-- 컬럼자체에 NULL 허용/비허용을 제어하는 성질 변경의 형태로 인식

-- MODIFY (수정하다) 구문을 사용해서 NULL 제어
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; -- NOT NULL 적용
...
--> DEPT_ID의 데이터 타입이 CHAR(2) 이므로 영어+숫자 2글자까지 저장 가능 D10은 3바이트
...
-- DROP TABLE 테이블명 [CACADE CONSTRAINTS];
...
-- 부모인 TB1 테이블을 삭제하려 할 때
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
--> 해결방법
-- 1) 자식, 부모테이블 순서로 삭제
-- 2) ALTER 를 이용해 FK 제약조건 삭제 후 TB1 사제

...
-- 그룹함수는 단 한개의 결과값만 산출하기 때문에 그룹이 여러개일 경우 오류
-- 여러개의 결과값을 산출하기 위해 그룹함수가 적용된 그룹의 기준을 ORDER BY 절에 기술
...
FROM EMPLOYEE
GROUP BY DEPT_CODE;
-- DEPT_CODE를 그룹으로 묶어, 그 그룹의 급여 합계를 구함
...
-- 그룹함수가 적용되지 않은 컬럼을
-- 모두 GROUP BY 절에 작성되어야 함 

-- * 여러 컬럼을 묶어서 그룹 지정 가능( 그룹 내 그룹 ) *

-- 부서별로 같은 직급인 사원의 인원수 조회
...
-- *HAVING 절 : 그룹함수로 구해 올 그룹에 대한 조건을 설정
-- HAVING 절에는 그룹함수가 반드시 존재
-- HAVING 컬럼명 | 함수식 비교연산자 비교값

-- 부서별 평균 급여가 300만 이상인 부서의
-- 부서코드, 평균급여 조회
-- 부서코드 오름차순
...
-- ROLLUP : GROUP BY절에서 가장 먼저 작성된 컬럼 중간 집계를 처리
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_CODE, JOB_CODE)
ORDER BY 1;

-- CUBE : GROUP BY절에 작성된 모든 컬럼 중간 집계 처리
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
 ...
 * 
 * UNION(합집합) : 두 SELECT 결과를 하나로 합침(중복 1번만 작성)
 * 
...
-- 주의사항
-- SELECT 문들은 조회하는 컬럼의 타입, 개수가 모두 동일해야 함

-- (1번 SELECT 문)부서코드가 'D5'인 사원의 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION -- / INTERSECT / UNION ALL / MINUS
-- (2번 SELECT 문)급여가 300만원 초과인 사원의 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
